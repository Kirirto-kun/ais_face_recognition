...
def generate_image(user_data, output_path):
    ...
    draw.text((header_x, 30), header_text+"\nпо уважительной причине", fill="black", font=header_font)
    ...
    if photo_blob:
        ...
        photo_y = 150  # Фото сразу под заголовком
    ...
    try:
        stamp = Image.open("stampgood.png").convert("RGBA")
...
def main_good_async(iin=IIN, output_path=OUTPUT_IMAGE, callback=None):
    """
    Асинхронно генерирует сертификат об опоздании по уважительной причине.
...
# Запуск только если файл исполняется напрямую, а не импортируется
if __name__ == "__main__":
    ...
    test_executor = main_good_async(IIN, callback=on_complete)
